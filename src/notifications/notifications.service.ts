...
export class NotificationsService {
  ...
  async onSosTriggered(payload: {
    ...
    // Confirms receipt to the reporter first - this alone was never
    // the gap; nothing downstream of it was.
    await this.safeNotify(
      payload.userId,
    ...
    // Real fix: every safety-ops staff member (admin/dispatcher/etc.)
    // gets an actual, queued, retried push+in-app notification - not
    // just whoever happens to be watching the live admin dashboard
    // right now (TrackingGateway.broadcastSosAlert() handles that
    // separately, for the people actively watching).
    const opsStaff = await this.usersService.findByRoles(SAFETY_OPS_ROLES);
    const locationNote = payload.lat != null && payload.lng != null ? ` Location: ${payload.lat.toFixed(5)}, ${payload.lng.toFixed(5)}.` : '';
    const opsBody = `${payload.reporterRole ?? 'User'} ${payload.reporterName} triggered SOS.${locationNote} Incident ${payload.incidentId}.`;
    await Promise.all(
      opsStaff.map((staff) =>
        this.safeNotify(
          staff.id,
